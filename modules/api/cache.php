...
class cache extends APICall {
    ...
    function handle() {
        ...
        if ($group["recache_count"] > 0) {
            ...
            AjaxResponse::create()->response($mysql->exec(QUERY_GROUP_UPDATE, RETURN_IGNORE,
                array("name" => $group["name"], "count" => $group["recache_count"] - 1)),
                array("info" => array("left" => $group["recache_count"] - 1, "cached_last" => $cached_last, "cached_until" => $cached_until)));
        } else {
